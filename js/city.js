(function(){'use strict';
  ...
  const cityName = document.body.dataset.cityName || '';
  const stateName = document.body.dataset.state || '';
  const container = $('#city-studies-list');
  const paginationEl = $('#city-pagination');
  if (!container) return;
  ...
  function doRender() {
    ...
    if (!pageItems.length) {
      container.innerHTML = '<div class="empty-state"><div class="es-icon">!</div><h3>No studies found</h3><p>Check back later for new clinical trials in ' + escape(cityName) + ', ' + escape(stateName) + '.</p></div>';
    ...
    const countEl = $('#city-results-count');
